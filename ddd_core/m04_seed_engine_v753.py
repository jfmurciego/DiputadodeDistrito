...
VERSIÓN: 7.5.3
NOMBRE: Puerta mínima y lectura GeoJSON nativa
FECHA: 2026-09-11
...
CAMBIOS: usa el postproceso v7.4.8 con lectura JSON nativa, evitando que OGR descarte propiedades StringList.\nANTERIOR: ddd_core/m04_seed_engine_v752.py
"""
...
POST_PATH=ROOT/'ddd_core'/'m04_seed_engine_v748.py'
...
def _normalize_unit_property_for_ogr(path):
    p=Path(path)
    if p.suffix.lower()!='.zip': return 0
    ...
    for feature in data.get('features',[]):
        ...
        if isinstance(value,list):
            if len(value)!=1:
                raise SystemExit(f'M04 v7.5.3: ddd_unit_id multivaluado no normalizable: {value!r}')
            props['ddd_unit_id']=str(value[0]);changed+=1
    if changed:
...
def main():
    ...
    base=_load(BASE_PATH,'ddd_m04_seed_engine_v745_for_v753')
    ...
    changed=_normalize_unit_property_for_ogr(out)
    if changed: print(f'[Módulo 4] normalizados ddd_unit_id OGR-safe={changed}')
    post=_load(POST_PATH,'ddd_m04_seed_engine_v747_post_for_v753')
