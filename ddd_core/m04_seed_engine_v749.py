...
def postprocess(params_path):
    ...
    for prov in sorted(quota):
        ...
        for _ in range(50):
            ...
            if not cs:
                raise SystemExit(
                    f"M04 v7.4.9: provincia {prov} agregadamente imposible y sin ajuste núcleo-residuo válido; "
    ...
    if g[did].nunique() != K:
        raise SystemExit(f"M04 v7.4.9: K alterado {g[did].nunique()} != {K}")
    counts = {str(x[provf].iloc[0]).zfill(2): 0 for _, x in g.groupby(did)}
    for _, x in g.groupby(did):
        counts[str(x[provf].iloc[0]).zfill(2)] += 1
    if counts != quota:
        raise SystemExit(f"M04 v7.4.9: cuotas provinciales alteradas {counts} != {quota}")

    for d, x in g.groupby(did):
        ns = set(x[idf].astype(str))
        if not connected(ns, adj):
            raise SystemExit(f"M04 v7.4.9: distrito {d} desconectado")
        if x[provf].astype(str).str.zfill(2).nunique() != 1:
            raise SystemExit(f"M04 v7.4.9: distrito {d} cruza provincia")
        if bool(x["ddd_closed_urban"].all()):
            p = int(x["district_pop_section"].sum())
            if not (lo <= p <= hi):
                raise SystemExit(f"M04 v7.4.9: núcleo cerrado {d} fuera de tolerancia: {p}")
    ...
    for prov in sorted(quota):
        ...
        if not (len(open_ids) * lo - 1e-9 <= open_pop <= len(open_ids) * hi + 1e-9):
            raise SystemExit(f"M04 v7.4.9: factibilidad residual incumplida en {prov}")

    if hard:
        raise SystemExit(f"M04 v7.4.9: {hard} violaciones duras tras ajuste")
    ...
    print(
        f"[Módulo 4] OK v7.4.9 K={K} ajustes_factibilidad={len(adjustments)} "
