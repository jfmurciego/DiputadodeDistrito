...
VERSIÓN: 7.4.8
NOMBRE DE VERSIÓN: Ajuste mínimo con lectura GeoJSON nativa
FECHA: 2026-09-11
...
CAMBIOS: lee GeoJSON comprimido mediante JSON/GeoDataFrame.from_features para conservar propiedades de tipo lista que OGR omite; normaliza listas unitarias ddd_unit_id. Mantiene el ajuste mínimo de v7.4.7. Solo actúa cuando open_pop queda fuera de N_abiertos×[target−tol,target+tol], selecciona el movimiento fronterizo mínimo que repara el déficit y se detiene inmediatamente al recuperar factibilidad. Recalcula hard/outside/min/max en vez de heredar métricas del motor base.
...
ANTERIOR: ddd_core/m04_seed_engine_v747.py
"""
    ...
    if p.suffix.lower() == ".zip":
        with zipfile.ZipFile(p) as z:
            ...
            if "ddd_unit_id" in gdf.columns:
                gdf["ddd_unit_id"] = gdf["ddd_unit_id"].map(
                    lambda value: str(value[0]) if isinstance(value, list) and len(value) == 1 else value
                )
            return gdf
...
def postprocess(params_path):
    ...
    for prov in sorted(quota):
        for _ in range(50):
            ...
            if not cs:
                raise SystemExit(
                    f"M04 v7.4.8: provincia {prov} agregadamente imposible y sin ajuste núcleo-residuo válido; "
            ...
            raise SystemExit(f"M04 v7.4.8: demasiados ajustes en provincia {prov}")
    ...
    if g[did].nunique() != K:
        raise SystemExit(f"M04 v7.4.8: K alterado {g[did].nunique()} != {K}")
    counts = {str(x[provf].iloc[0]).zfill(2): 0 for _, x in g.groupby(did)}
    for _, x in g.groupby(did):
        counts[str(x[provf].iloc[0]).zfill(2)] += 1
    if counts != quota:
        raise SystemExit(f"M04 v7.4.8: cuotas provinciales alteradas {counts} != {quota}")
    ...
    for d, x in g.groupby(did):
        ...
        if x[provf].astype(str).str.zfill(2).nunique() != 1:
            raise SystemExit(f"M04 v7.4.8: distrito {d} cruza provincia")
        if bool(x["ddd_closed_urban"].all()):
            p = int(x["district_pop_section"].sum())
            if not (lo <= p <= hi):
                raise SystemExit(f"M04 v7.4.8: núcleo cerrado {d} fuera de tolerancia: {p}")
    ...
    for prov in sorted(quota):
        ...
        if not (len(open_ids) * lo - 1e-9 <= open_pop <= len(open_ids) * hi + 1e-9):
            raise SystemExit(f"M04 v7.4.8: factibilidad residual incumplida en {prov}")

    if hard:
        raise SystemExit(f"M04 v7.4.8: {hard} violaciones duras tras ajuste")
    ...
    print(
        f"[Módulo 4] OK v7.4.8 K={K} ajustes_factibilidad={len(adjustments)} "
