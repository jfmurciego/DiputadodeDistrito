...
VERSIÓN: 7.4.11
NOMBRE DE VERSIÓN: Ajuste mínimo con normalización secuencial robusta
FECHA: 2026-09-11
...
CAMBIOS: normaliza también arrays/tuplas unitarios devueltos por GeoDataFrame.from_features antes de exportar. Mantiene el ajuste mínimo de v7.4.8. Solo actúa cuando open_pop queda fuera de N_abiertos×[target−tol,target+tol], selecciona el movimiento fronterizo mínimo que repara el déficit y se detiene inmediatamente al recuperar factibilidad. Recalcula hard/outside/min/max en vez de heredar métricas del motor base.
...
ANTERIOR: ddd_core/m04_seed_engine_v748.py
"""
...
BASE_ENGINE = ROOT / "ddd_core" / "m04_seed_engine_v7410.py"
            ...
            if "ddd_unit_id" in gdf.columns:
                def scalar_unit(value):
                    ...
                    if isinstance(value, (list, tuple)) and len(value) == 1:
                        return str(value[0])
                    raise SystemExit(f"M04 v7.4.9: ddd_unit_id no escalar: {value!r}")
...
def postprocess(params_path):
    ...
    for prov in sorted(quota):
        ...
        for _ in range(50):
            ...
            if not cs:
                unresolved_population.append({
                    "province": prov,
                    "reason": "NO_FEASIBLE_CORE_RESIDUAL_ADJUSTMENT",
                    "open_population": int(open_pop),
            ...
            unresolved_population.append({
                "province": prov,
                "reason": "FEASIBILITY_ADJUSTMENT_BUDGET_EXHAUSTED",
            })
    ...
    for d, x in g.groupby(did):
        if bool(x["ddd_closed_urban"].all()):
            ...
            if not (lo <= p <= hi):
                closed_core_population_exceptions.append({
                    "district_id": int(d),
                    "population": p,
                    "target_min": float(lo),
                    "target_max": float(hi),
    ...
    residual_feasibility_exceptions = []
    for prov in sorted(quota):
        if prov in floor_exempt:
            continue
        ids = sorted(g.loc[g[provf] == prov, did].unique())
        ...
        open_pop = sum(dpop(d) for d in open_ids)
        open_min, open_max = len(open_ids) * lo, len(open_ids) * hi
        if not (open_min - 1e-9 <= open_pop <= open_max + 1e-9):
            residual_feasibility_exceptions.append({
                "province": prov,
                "open_population": int(open_pop),
                "open_min": float(open_min),
                "open_max": float(open_max),
    ...
    hard = diagnostics["hard_population_violations"]
    counts = diagnostics["province_counts"]
    ...
    rep.update({
        ...
        "districts_below_floor": diagnostics["districts_below_floor"],
        "districts_above_ceiling": diagnostics["districts_above_ceiling"],
        "population_districts": diagnostics["population_districts"],
        "population_baseline_status": "PASS" if hard == 0 else "PASS_WITH_EXCEPTIONS",
        "population_floor_exempt_partitions": sorted(floor_exempt),
        "province_counts": counts,
        "province_feasibility_adjustments": adjustments,
        "unresolved_population_feasibility": unresolved_population,
        "closed_core_population_exceptions": closed_core_population_exceptions,
        "residual_feasibility_exceptions": residual_feasibility_exceptions,
    })
    ...
    rep["rules"]["population_limits_are_diagnostic_at_m04"] = True
    if report_path:
        Path(report_path).write_text(json.dumps(rep, ensure_ascii=False, indent=2), encoding="utf-8")
    print(
        f"[Módulo 4] OK v7.4.11 K={K} ajustes_factibilidad={len(adjustments)} "
