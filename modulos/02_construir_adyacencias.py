...
VERSIÓN: 7.1.0
NOMBRE DE VERSIÓN: Puentes topológicos auditables
FECHA: 2026-09-11
QUÉ HACE: calcula adyacencias geométricas y, opcionalmente, incorpora puentes topológicos explícitos declarados en la configuración territorial.
POR QUÉ ES SEPARADO: la adyacencia depende de la cartografía y de excepciones territoriales verificables, no del algoritmo de reparto.
ESTADO: candidato CYL-02; compatible con Aragón si no se declaran puentes.
CAMBIOS: añade topology_bridges configurables, valida sus extremos y los marca en el JSONL; la geometría normal sigue usando touches/intersects sin cambios.
MOTIVO: representar enclaves administrativos y huecos cartográficos reales sin buffers globales ni excepciones hardcodeadas por territorio.
ANTERIOR: legacy/modulo02/02_construir_adyacencias_v7.0.1.py
"""
...
def _normalize_bridge(raw,valid_ids):
    if not isinstance(raw,dict):raise SystemExit("M02 topology_bridges debe contener objetos")
    u=str(require(raw.get("u"),"Puente sin u"));v=str(require(raw.get("v"),"Puente sin v"))
    if u==v:raise SystemExit(f"Puente inválido con extremos iguales: {u}")
    if u not in valid_ids or v not in valid_ids:raise SystemExit(f"Puente con extremo inexistente: {u}-{v}")
    a,b=(u,v) if u<v else (v,u)
    return {"u":a,"v":b,"edge_type":str(raw.get("edge_type","topology_bridge")),"reason":str(raw.get("reason","")),"source":str(raw.get("source","config"))}
...
def write_edges_jsonl(geometric_edges,bridges,out_path):
    ...
    for b in bridges:
        key=(b["u"],b["v"])
        if key not in geom:bridge_map[key]=b
    with outp.open("w",encoding="utf-8") as f:
        for u,v in sorted(geom):f.write(json.dumps({"u":u,"v":v,"edge_type":"geometric"},ensure_ascii=False)+"\n")
        for key in sorted(bridge_map):f.write(json.dumps(bridge_map[key],ensure_ascii=False)+"\n")
    return len(geom)+len(bridge_map),len(geom),len(bridge_map)

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--params",required=True);args=ap.parse_args();cfg=load_params_yaml(args.params);s2=module_cfg(cfg,"modulo_02_construir_adyacencias",legacy_step_key="step2_export_edges");in_geo=require(s2.get("in_geojson"),"Falta M02 entrada");id_field=require(s2.get("id_field"),"Falta M02 id");out_edges=require(s2.get("out_edges_jsonl"),"Falta M02 salida");gdf=load_geojson_any(in_geo);gdf[id_field]=gdf[id_field].astype(str);valid_ids=set(gdf[id_field]);geometric=list(iter_edges(gdf,id_field,s2.get("predicate","touches"),float(s2.get("min_shared_border_m",0)),float(s2.get("buffer_m",0)),float(s2.get("simplify_m",0)),int(s2.get("max_candidates",50)),int(s2.get("log_every",10000))));bridges=[_normalize_bridge(x,valid_ids) for x in (s2.get("topology_bridges",[]) or [])];n,ng,nb=write_edges_jsonl(geometric,bridges,out_edges);print(f"[Módulo 2] OK edges={n} geometric={ng} bridges={nb} out={out_edges}")
