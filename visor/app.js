...
async function bootstrap(){try{let response=await fetch("data/viewer-results.json",{cache:"no-cache"});if(!response.ok)throw new Error(`registro HTTP ${response.status}`);let registry=await response.json();RESULTS=registry.results||[]}catch(error){console.warn("Registro de ejecuciones no disponible; se intenta catálogo histórico.",error);const response=await fetch("data/public-products.json",{cache:"no-cache"});const registry=await response.json();RESULTS=registry.products.map(p=>({id:`static-${p.id}`,territory_id:p.id,territory_label:p.label,label:`${p.label} · producto histórico`,kind:"static",expected_districts:p.expected_districts,viewer_path:p.viewer_path,technical_status:p.technical_status||p.status,publication_status:p.publication_status||"BLOCKED",geometric_status:"LEGACY_OR_NOT_AUDITED"}))}const territories=[...new Map(RESULTS.map(r=>[r.territory_id,{label:r.territory_display_name||r.territory_label,code:r.autonomous_community_code_ine||"99"}])).entries()].sort((a,b)=>a[1].code.localeCompare(b[1].code)||a[1].label.localeCompare(b[1].label,"es"));territorySelect.replaceChildren(...territories.map(([id,meta])=>new Option(meta.label,id)));territorySelect.addEventListener("change",refreshResults);resultSelect.addEventListener("change",()=>loadResult(resultSelect.value));refreshResults()}
