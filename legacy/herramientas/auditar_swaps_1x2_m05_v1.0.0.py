        ...
        for left_size,right_size in ((1,2),(2,1)):
            for left in itertools.combinations(bd,left_size):
                for right in itertools.combinations(be,right_size):
                    ...
                    results.append({"districts":[d,e],"shape":f"{left_size}x{right_size}","units_d":list(left),"units_e":list(right),"population_d_package":lp,"population_e_package":rp,"municipalities_d":sorted({m for u in left for m in unit_mun[u]}),"municipalities_e":sorted({m for u in right for m in unit_mun[u]}),"new_pops":[pd,pe],"objective":list(obj),"reasons":reasons,"valid_improvement":not reasons})
