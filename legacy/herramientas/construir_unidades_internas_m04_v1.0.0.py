...
def main():
    ...
    for mun, x in sorted(g.groupby(a.municipality_field), key=lambda kv: str(kv[0])):
        ...
        parts, pvals, pobj = eng.rebalance(
            parts, adj, pop, avg, avg * 0.55, avg * 1.45, avg * 0.30, 30000,
        )

        ordered = sorted(zip(parts, pvals), key=lambda z: (min(str(n) for n in z[0]), z[1]))
        chunks = []
        for seq, (part, pval) in enumerate(ordered, start=1):
            ...
            exterior_sections = sorted(n for n in part if any(nb not in nodes for nb in adj.get(n, set())))
            chunks.append({"partition_unit": uid, "sections": sorted(part), "section_count": len(part), "population": int(pval), "ratio_to_target": float(pval / target), "touches_municipal_exterior": bool(exterior_sections), "exterior_gateway_sections": exterior_sections})

        report_municipalities.append({"municipality": mun, "population": int(mp), "ratio_to_target": float(mp / target), "chunk_count": len(chunks), "chunk_average_population": float(mp / len(chunks)), "chunk_average_ratio_to_target": float((mp / len(chunks)) / target), "rebalance_objective": list(pobj), "chunks": chunks})
    ...
    split_muns = {str(m): int(x[a.partition_unit_field].nunique()) for m, x in g.groupby(a.municipality_field) if x[a.partition_unit_field].nunique() > 1}
    result = {"version": "1.0.0", "K": a.k, "total_population": total, "target": target, "atomicity_ratio": a.atomicity_ratio, "atomicity_limit": atomic_limit, "chunk_ratio": a.chunk_ratio, "desired_chunk_population": desired_chunk, "partition_unit_field": a.partition_unit_field, "partition_unit_count": int(g[a.partition_unit_field].nunique()), "oversized_municipality_count": len(report_municipalities), "split_municipalities": split_muns, "municipalities": report_municipalities}
    ...
    print(json.dumps({"target": round(target, 3), "chunk_ratio": a.chunk_ratio, "partition_units": result["partition_unit_count"], "oversized_municipalities": result["oversized_municipality_count"], "split_municipalities": split_muns}, ensure_ascii=False))
