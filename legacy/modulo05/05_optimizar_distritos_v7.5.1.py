...
def main():
    ...
    if not str(in_path): raise SystemExit("M05 v7.5.1: falta in_geojson")
    if _ogr_can_read_unit(in_path):
        _run_base(str(params)); out_path = Path(str(s5.get("out_geojson", "")))
        report_path = Path(str(s5.get("out_report", ""))) if s5.get("out_report") else None
        meta = _apply_swap_polish(cfg, s5, out_path, report_path)
        print(f"[Módulo 5 wrapper] OK v7.5.1 swap_polish={meta.get('accepted_swaps', 0)} out={out_path}"); return
    ...
    with tempfile.TemporaryDirectory(prefix="ddd_m05_751_") as td_raw:
        ...
        cfg2 = copy.deepcopy(cfg); s52 = _module_cfg(cfg2); s52["in_geojson"] = str(tmp_input); s52["out_geojson"] = str(tmp_output); s52["out_report"] = str(tmp_report); cfg2.setdefault("modulos", {})["modulo_05_optimizar_distritos"] = s52
        tmp_params.write_text(yaml.safe_dump(cfg2, sort_keys=False, allow_unicode=True), encoding="utf-8")
        _run_base(str(tmp_params)); meta = _apply_swap_polish(cfg2, s52, tmp_output, tmp_report)
        final_out = Path(str(s5.get("out_geojson"))); final_report = Path(str(s5.get("out_report", ""))) if s5.get("out_report") else None
        final_out.parent.mkdir(parents=True, exist_ok=True); final_out.write_bytes(tmp_output.read_bytes())
        rep = json.loads(tmp_report.read_text(encoding="utf-8")) if tmp_report.exists() else {}; rep["version"] = "7.5.1"; rep["swap_polish"] = meta; rep["unit_id_normalization"] = {"applied": True,"reason": "OGR dropped ddd_unit_id; raw GeoJSON labels mapped to stable integer codes","units": len(unique),"code_to_original_label": code_to_label}
        if final_report: final_report.parent.mkdir(parents=True, exist_ok=True); final_report.write_text(json.dumps(rep, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"[Módulo 5 wrapper] OK v7.5.1 normalized_units={len(unique)} swap_polish={meta.get('accepted_swaps', 0)} out={final_out}")
