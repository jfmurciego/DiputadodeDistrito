...
VERSIÓN: 7.4.1
NOMBRE DE VERSIÓN: Identidad de unidad robusta frente a OGR
FECHA: 2026-09-11
...
CAMBIOS: añade fallback de lectura JSON pura cuando OGR interpreta una propiedad GeoJSON como StringList y la omite. En entradas normales delega byte-a-byte la lógica al motor congelado v7.4.0.
MOTIVO: EXT-03 demostró que una semilla M04 válida podía ser ilegible para M05 por metadatos/tipado OGR de `ddd_unit_id`; el problema es de transporte de esquema, no de optimización.
ANTERIOR: legacy/modulo05/05_optimizar_distritos_v7.4.0.py
"""
...
def _load_base():
    ...
    if spec is None or spec.loader is None:
        raise SystemExit(f"M05 v7.4.1: no se puede cargar {BASE_ENGINE}")
...
def _normalise_label(v):
    if isinstance(v, list):
        if len(v) != 1:
            raise SystemExit(f"M05 v7.4.1: ddd_unit_id multivaluado no normalizable: {v!r}")
        v = v[0]
    if v is None:
        raise SystemExit("M05 v7.4.1: ddd_unit_id nulo en GeoJSON crudo")
    return str(v)
...
def main():
    ...
    if not in_path:
        raise SystemExit("M05 v7.4.1: falta in_geojson")
    ...
    for f in data.get("features", []):
        labels.append(_normalise_label((f.get("properties") or {}).get("ddd_unit_id")))
    ...
    with tempfile.TemporaryDirectory(prefix="ddd_m05_741_") as td:
        td = Path(td)
        ...
        mods = cfg2.setdefault("modulos", {})
        mods["modulo_05_optimizar_distritos"] = s52
        ...
        print(f"[Módulo 5 wrapper] OK v7.4.1 normalized_units={len(unique)} out={final_out}")
