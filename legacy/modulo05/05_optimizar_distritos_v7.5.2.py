...
def _apply_swap_polish(cfg: dict, s5: dict, out_path: Path, report_path: Path | None):
    ...
    if max_swaps == 0:
        ...
        meta = swap_polish(cfg=cfg, graph_path=graph_path, geojson_path=out_path, out_geojson_path=out_path, max_swaps=max_swaps)
        meta["enabled"] = True
...
def main():
    ...
    params = Path(args.params).resolve(); cfg = load_params_yaml(str(params)); s5 = _module_cfg(cfg)
    in_path = Path(str(s5.get("in_geojson", "")))
    if not str(in_path): raise SystemExit(f"M05 v{WRAPPER_VERSION}: falta in_geojson")
    if _ogr_can_read_unit(in_path):
        _run_base(str(params)); out_path = Path(str(s5.get("out_geojson", ""))); report_path = Path(str(s5.get("out_report", ""))) if s5.get("out_report") else None
        meta = _apply_swap_polish(cfg, s5, out_path, report_path); print(f"[Módulo 5 wrapper] OK v{WRAPPER_VERSION} swap_polish={meta.get('accepted_swaps', 0)} out={out_path}"); return
    data, inner = _raw_geojson(in_path); labels = [_normalise_label((f.get("properties") or {}).get("ddd_unit_id")) for f in data.get("features", [])]
    unique = sorted(set(labels)); label_to_code = {label: i + 1 for i, label in enumerate(unique)}; code_to_label = {str(code): label for label, code in label_to_code.items()}
    for f, label in zip(data.get("features", []), labels): f.setdefault("properties", {})["ddd_unit_id"] = int(label_to_code[label])
    with tempfile.TemporaryDirectory(prefix="ddd_m05_752_") as td_raw:
        td = Path(td_raw); tmp_input = td / "m05_input.geojson.zip"; tmp_output = td / "m05_output.geojson.zip"; tmp_report = td / "m05_report.json"; tmp_params = td / "params.yaml"
        _write_zip_json(data, tmp_input, "m05_input.geojson"); cfg2 = copy.deepcopy(cfg); s52 = _module_cfg(cfg2); s52["in_geojson"] = str(tmp_input); s52["out_geojson"] = str(tmp_output); s52["out_report"] = str(tmp_report); cfg2.setdefault("modulos", {})["modulo_05_optimizar_distritos"] = s52; tmp_params.write_text(yaml.safe_dump(cfg2, sort_keys=False, allow_unicode=True), encoding="utf-8")
        _run_base(str(tmp_params)); meta = _apply_swap_polish(cfg2, s52, tmp_output, tmp_report); final_out = Path(str(s5.get("out_geojson"))); final_report = Path(str(s5.get("out_report", ""))) if s5.get("out_report") else None; final_out.parent.mkdir(parents=True, exist_ok=True); final_out.write_bytes(tmp_output.read_bytes())
        normalization = {"applied": True, "reason": "OGR dropped ddd_unit_id; raw GeoJSON labels mapped to stable integer codes", "units": len(unique), "code_to_original_label": code_to_label}
        if final_report:
            final_report.parent.mkdir(parents=True, exist_ok=True); final_report.write_bytes(tmp_report.read_bytes()); _merge_report_metadata(final_report, swap_meta=meta, normalization=normalization)
        print(f"[Módulo 5 wrapper] OK v{WRAPPER_VERSION} normalized_units={len(unique)} swap_polish={meta.get('accepted_swaps', 0)} out={final_out}")
