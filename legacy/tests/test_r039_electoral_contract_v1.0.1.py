...
class R039ElectoralContract(unittest.TestCase):
    def test_contrato_aragon_verifica_hash_y_todas_las_siglas(self):
        ...
        self.assertEqual(set(m07), {"in_geojson", "section_id_field", "district_field", "election_contract", "out_district_party_csv", "out_district_summary_csv", "out_reconciliation_report", "out_sections_enriched_geojson"})
        ...
        if source_path.is_file():
            contract, parties = load_election_contract(contract_path, project_root=ROOT, expected_territory_id="aragon")
            frames = [M07.read_results(source["resolved_path"], source["adapter"], "CUSEC_KEY", parties) for source in contract["sources"]]
            ...
            self.assertIn(f"{source_record['sha256']}  {source_record['path']}", manifest)
            dictionary_record = declared["party_dictionary"]
            dictionary_path = ROOT / dictionary_record["path"]
            self.assertEqual(hashlib.sha256(dictionary_path.read_bytes()).hexdigest(), dictionary_record["sha256"])
    ...
    def test_checksum_alterado_bloquea_antes_de_leer_votos(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            dictionary.write_text(json.dumps({"schema_family":"ddd-party-dictionary","schema_version":"1.0.0","unknown_party_policy":"reject","parties":[{"canonical_id":"P","display_name":"P"}]}), encoding="utf-8")
            digest = hashlib.sha256(dictionary.read_bytes()).hexdigest()
            contract = root / "election.json"
            contract.write_text(json.dumps({"schema_family":"ddd-election","schema_version":"1.0.0","election_id":"synthetic-1","territory_id":"synthetic","title":"Elección sintética","election_date":"2026-01-01","input_mode":"verifiable_file","boundary_independence":True,"sources":[{"path":"votes.csv","sha256":"0"*64,"publisher":"fixture","source_url":"https://example.invalid","retrieved_at":"2026-01-01","adapter":{"kind":"long_csv","section_field":"section","party_field":"party","votes_field":"votes"}}],"party_dictionary":{"path":"parties.json","sha256":digest},"reconciliation":{"policy":"fail_unless_declared"}}), encoding="utf-8")
    ...
    def test_alias_ambiguo_y_partido_desconocido_se_rechazan(self):
        with self.assertRaisesRegex(ValueError, "alias ambiguo"):
            PartyDictionary({"schema_family":"ddd-party-dictionary","schema_version":"1.0.0","unknown_party_policy":"reject","parties":[{"canonical_id":"A","display_name":"Partido"},{"canonical_id":"B","display_name":" partido  "}]})
        dictionary = PartyDictionary({"schema_family":"ddd-party-dictionary","schema_version":"1.0.0","unknown_party_policy":"reject","parties":[{"canonical_id":"A","display_name":"Árbol","aliases":[" A "]}]})
