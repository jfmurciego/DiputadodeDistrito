#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""DDD Step 3 — Build adjacency graph + diagnostics (v6.0 unified params)

Run:
  python scripts/ddd_step3_build_graph_v6_params.py --params params/ddd_params.yaml

Reads YAML:
  steps.step3_build_graph.in_geojson
  steps.step3_build_graph.in_edges_jsonl
  steps.step3_build_graph.id_field
  steps.step3_build_graph.pop_field
  steps.step3_build_graph.out_graph_json
  steps.step3_build_graph.out_report (optional)

Output graph format (JSON):
  {
    "nodes": [{"id": "...", "pop": 123, "attrs": {...}}, ...],
    "edges": [{"u":"...","v":"..."}, ...]
  }
"""
...
def load_edges_jsonl(path_str: str) -> List[Tuple[str,str]]:
    p = Path(path_str).expanduser().resolve()
    if not p.exists():
        raise FileNotFoundError(f"Edges JSONL no encontrado: {p}")
    edges = []
...
def main():
    ap = argparse.ArgumentParser(description="DDD Step3 (unified params).")
    ...
    if id_field not in gdf.columns:
        raise SystemExit(f"GeoJSON sin id_field '{id_field}'. Columnas: {list(gdf.columns)[:40]}")
    if pop_field not in gdf.columns:
        raise SystemExit(f"GeoJSON sin pop_field '{pop_field}'. Columnas: {list(gdf.columns)[:40]}")

    df = gdf[[id_field, pop_field]].copy()
    df[id_field] = df[id_field].astype(str)
    df[pop_field] = pd.to_numeric(df[pop_field], errors="coerce").fillna(0).astype("int64")

    nodes = [{"id": rid, "pop": int(pop)} for rid, pop in zip(df[id_field].tolist(), df[pop_field].tolist())]
    pop_map = {n["id"]: n["pop"] for n in nodes}

    edges = load_edges_jsonl(in_edges)
    edges_f = [{"u": u, "v": v} for (u,v) in edges if (u in pop_map and v in pop_map)]
    ...
    report = {
        "step": "step3_build_graph",
        "nodes": int(len(nodes)),
        "edges": int(len(edges_f)),
        "isolated": int(isolated),
        "total_pop": total_pop,
        "id_field": id_field,
        "pop_field": pop_field,
        "in_geojson": in_geo,
        "in_edges_jsonl": in_edges,
        "out_graph_json": out_graph,
    }
    ...
    print(f"[Step3] OK nodes={len(nodes)} edges={len(edges_f)} isolated={isolated} out={out_graph}")
