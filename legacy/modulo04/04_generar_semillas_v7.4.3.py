...
VERSIÓN: 7.4.3
NOMBRE DE VERSIÓN: Pasarelas topológicas protegidas
FECHA: 2026-09-11
FUNCIÓN: construir exactamente K distritos dentro de sus provincias, preservando municipios completos mientras sean compatibles con el contrato territorial y dividiendo municipios sobredimensionados sin romper las pasarelas que sostienen la conectividad administrativa.
...
CAMBIOS: protege en el residuo municipal abierto todos los extremos de topology_bridges que caen dentro de un municipio dividido; la partición global solo se acepta si una única pieza residual contiene todas las pasarelas protegidas; la extracción secuencial prohíbe cerrar esos nodos. Añade diagnóstico explícito de componentes en el grafo de unidades abiertas.
MOTIVO: CYL-03 v7.4.2 mostró que cerrar una sección-pasarela puede desconectar un enclave del resto de su provincia aunque el grafo M03 sea íntegro. La conectividad virtual declarada en M02 debe sobrevivir a M04.
ANTERIOR: legacy/modulo04/04_generar_semillas_v7.4.2.py
"""
...
def find_closed_core(rem,desired,lo,hi,adj,w,left_after,protected=None,max_starts=800):
    rem=set(rem);protected=set(protected or ());rp=sum(w[n] for n in rem);starts=sorted(rem-protected,key=lambda n:(sum(v in rem for v in adj[n]),w[n],str(n)))[:max_starts];best=None
    for s in starts:
        order=[];seen={s};q=collections.deque([s]);cum=0
        while q:
            u=q.popleft();order.append(u);cum+=w[u]
            if cum>=lo:
                ...
                if not (ch&protected) and cum<=hi and connected(ch,adj) and rest_connected and future_ok:
                    score=(abs(cum-desired),cum,len(ch),str(s))
                    if best is None or score<best[0]:best=(score,ch)
                if cum>hi:break
            for v in sorted(adj[u],key=str):
...
def split_sequential(nodes,target,tol,adj,w,label='',protected=None):
    rem=set(nodes);protected=set(protected or ());mp=sum(w[n] for n in rem);lo=target-tol;hi=target+tol;n_closed=max(1,int(math.ceil(max(0.0,mp-hi)/hi)));closed=[]
    for idx in range(n_closed):
        ...
        desired=min(max(target,core_min),core_max);core=find_closed_core(rem,desired,core_min,core_max,adj,w,left_after,protected)
        if core is None:raise SystemExit(f'M04: no se puede extraer núcleo municipal factible {label}; pop_rem={rp} protected={sorted(protected)} rango=[{core_min:.2f},{core_max:.2f}]')
...
def partition_oversized_municipality(nodes,target,tol,adj,w,label='',protected=None):
    nodes=set(nodes);protected=set(protected or ());mp=sum(w[n] for n in nodes);lo=target-tol;hi=target+tol;q=max(2,int(round(mp/target)));avg=mp/q
    if lo<=avg<=hi and q<=len(nodes):
        ...
        if all(lo<=x<=hi for x in pvals):
            eligible=[i for i,p in enumerate(parts) if protected.issubset(p)] if protected else list(range(len(parts)))
            if eligible:
                residual_i=max(eligible,key=lambda i:(sum(1 for n in parts[i] for nb in adj[n] if nb not in nodes),-abs(pvals[i]-target),-i))
    ...
    return split_sequential(nodes,target,tol,adj,w,label,protected)
...
def main():
    ...
    for prov in sorted(quota):
        ...
        for mun,nodes in sorted(mun_nodes.items()):
            ...
            label=f'{prov}/{mun}/{mname}';protected=bridge_nodes&set(nodes);cores,residual,mode=partition_oversized_municipality(nodes,target,tol,adj,pop,label,protected)
            ...
            oversized_report.append({'municipality':mun,'municipality_name':mname,'population':mp,'partition_mode':mode,'protected_gateway_nodes':sorted(protected),'closed_cores':len(cores),'closed_core_populations':[sum(pop[n] for n in p) for p in cores],'residual_population':sum(pop[n] for n in residual),'atomic_limit':atomic_limit})
    ...
    write_geo(g,out);rep={'module':'04','version':'7.4.3','K':K,'total_pop':int(total),'target':target,'floor':floor,'cap':cap,'tolerance':tol,'municipality_atomicity_limit_ratio':atomic_ratio,'municipality_atomicity_limit':atomic_limit,'topology_gateway_nodes':sorted(bridge_nodes),'min_pop':int(pops.min()),'max_pop':int(pops.max()),'outside_target_tolerance':outside,'hard_population_violations':hard,'province_counts':prov_counts,'province_districts':prov_report,'assigned_missing':0,'rules':{'single_province':True,'municipality_atomic_until_configured_limit':True,'topology_bridge_gateways_preserved_in_open_residual':True,'oversized_municipality_global_partition_when_feasible':True,'oversized_municipality_core_residual_fallback':True,'district_contiguity_preexport':True}}
    if report_path:Path(report_path).write_text(json.dumps(rep,ensure_ascii=False,indent=2),encoding='utf-8')
    print(f'[Módulo 4] OK v7.4.3 K={K} provincias={prov_counts} hard=0 outside_tol={outside} min={int(pops.min())} max={int(pops.max())} out={out}')
