#!/usr/bin/env python3
"""Checkpoints semánticos G10 v1.0.0: reanudación segura sin recalcular evidencia."""
from __future__ import annotations
from dataclasses import dataclass
from pathlib import Path
from typing import Any, Mapping, Sequence
from g10.stages import BY_ID, describe
...
def validate_checkpoint_inventory(inventory:Mapping[str,Any])->None:
    ...
    for i,item in enumerate(checkpoints):
        ...
        stage_id=item.get("stage_id"); key=(stage_id,item.get("source_run"))
        if stage_id not in BY_ID: errors.append(f"checkpoints[{i}].stage_id desconocido")
        if not isinstance(item.get("source_run"),str) or not item.get("source_run"): errors.append(f"checkpoints[{i}].source_run obligatorio")
        ...
        if key in seen: errors.append(f"checkpoint duplicado: {key}")
        seen.add(key)
