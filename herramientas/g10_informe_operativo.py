#!/usr/bin/env python3
"""Informe operativo G10 v1.0.0: estado durable legible y de máquina."""
...
def build(state,factual):
    ...
    next_step={"kind":"REVIEW_BLOCKED_TASKS","tasks":[x["task_id"] for x in blocked],"message":"Revisar sólo las tareas bloqueadas; las SUCCESS o REUSED no se repiten."} if blocked else {"kind":"WAIT_FOR_MATERIAL_CHANGE","tasks":[],"message":"No hay cálculo territorial autorizado. Relanzar G10 únicamente si cambia una huella, contrato, fuente o producto."}
    return {"schema_version":"1.0.0","generated_at":str(date.today()),"phase":factual.get("phase"),"phase_status":factual.get("status"),"territories":territories,"g10":{"state_schema_version":state.get("schema_version"),"stage_catalog_version":state.get("stage_catalog_version"),"latest_task_count":len(records),"status_counts":dict(sorted(Counter(x.get("status","UNKNOWN") for x in records).items())),"latest_tasks":records},"next_reengagement":next_step,"execution_policy":"No repetir M01–M06 ni abrir territorios nuevos sin cambio material y orden explícita."}

def markdown(report):
    lines=["# Estado operativo G10","","Generado: "+report["generated_at"],"Fase: "+str(report["phase"])+" — "+str(report["phase_status"]),"","## Territorios","","| Territorio | Estado | Distritos |","|---|---|---:|"]
...
def main():
    p=argparse.ArgumentParser();p.add_argument("--state",required=True);p.add_argument("--factual",required=True);p.add_argument("--output-json",required=True);p.add_argument("--output-md",required=True)
