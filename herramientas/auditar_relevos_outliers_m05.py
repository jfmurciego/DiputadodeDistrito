...
def main():
    ...
    total=sum(pop.values());K=len(d_units);target,floor,cap,tol=hard_limits(cfg,k=K,total_pop=total);lower=target-tol;upper=target+tol;base_obj=objective(dpop,target,floor,cap,tol)
