...
VERSIÓN: 1.0.1
NOMBRE: Probe de movimientos topológicos M04 — fallback GeoJSON crudo
FECHA: 2026-09-11
...
CAMBIOS: recupera `ddd_unit_id` desde el GeoJSON crudo por `id_field` cuando OGR descarta la propiedad.
MOTIVO DEL CAMBIO: EXT-11 confirmó de nuevo `unsupported OGR type: 10` en M04; EXT-12 debe ser robusto a
la misma representación antes de ejecutarse.
ANTERIOR: legacy/herramientas/auditar_movimientos_topologicos_m04_v1.0.0.py
"""
...
def load_geo_with_unit(path,idf):
    data,raw=read_raw(path);g=gpd.read_file(io.BytesIO(raw))
    ...
    for f in data.get('features',[]):
        ...
        if isinstance(v,list):
            ...
            v=v[0]
        mapping[str(k)]=str(v)
    if not mapping:raise SystemExit('No se pudo recuperar ddd_unit_id del GeoJSON crudo')
    g[idf]=g[idf].astype(str);g['ddd_unit_id']=g[idf].map(mapping)
    if g['ddd_unit_id'].isna().any():raise SystemExit('ddd_unit_id incompleto tras fallback crudo')
    return g
...
def articulation_signature(d_units,uadj,unit_pop,target):
    ...
    for us in d_units.values():
        ...
        for cut in us:
            ...
            arts+=1;small=min(sum(unit_pop[u] for u in c) for c in comps);vulnerable_sum+=small;max_vulnerable=max(max_vulnerable,small)
...
def pop_objective(dpop,target,floor,cap,tol):
    vals=list(dpop.values());hard=sum(p<floor or p>cap for p in vals);outside=sum(abs(p-target)>tol for p in vals);mx=max(abs(p-target)/target for p in vals);sq=sum(((p-target)/target)**2 for p in vals);return (hard,outside,round(mx,12),round(sq,12))
...
def main():
    ...
    idf=s5.get('id_field') or s4.get('id_field','CUSEC_KEY');did=s5.get('district_field','district_id');provf=s5.get('province_field') or s4.get('province_field','CPRO');munf=s5.get('municipality_field','CUMUN');g=load_geo_with_unit(a.geojson,idf)
    ...
    total=sum(pop.values());K=len(d_units);target=total/K;floor=target*float(val.get('population_floor_ratio',.8));cap=target*float(val.get('population_cap_ratio',1.75));tol=target*float(val.get('target_tolerance_ratio',.12));cur_pop=pop_objective(dpop,target,floor,cap,tol);cur_frag=articulation_signature(d_units,uadj,unit_pop,target);cands=[]
    for u,a0 in sorted(unit_dist.items(),key=lambda x:str(x[0])):
        ...
        for b in neigh:
            ...
            trial_units={d:set(us) for d,us in d_units.items()};trial_units[a0].remove(u);trial_units[b].add(u);trial_pop=dict(dpop);trial_pop[a0]=pa;trial_pop[b]=pb;pobj=pop_objective(trial_pop,target,floor,cap,tol);frag=articulation_signature(trial_units,uadj,unit_pop,target)
            if frag>=cur_frag:continue
            strict=pobj<=cur_pop;guarded=(pobj[0]==0 and pobj[1]<=cur_pop[1] and pobj[2]<=cur_pop[2])
    ...
    result={'current_population_objective':list(cur_pop),'current_fragility_signature':list(cur_frag),'valid_topology_improving_moves':len(cands),'strict_population_nonworsening':sum(x['strict_population_nonworsening'] for x in cands),'guarded_population':sum(x['guarded_population'] for x in cands),'best':cands[:a.top]};out=Path(a.out);out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps({k:result[k] for k in ('current_population_objective','current_fragility_signature','valid_topology_improving_moves','strict_population_nonworsening','guarded_population')},ensure_ascii=False));print(json.dumps(result['best'][:5],ensure_ascii=False))
