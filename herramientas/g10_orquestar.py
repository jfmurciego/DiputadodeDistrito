#!/usr/bin/env python3
"""CLI G10 v1.1.0: planifica, admite, verifica evidencia y materializa estado."""
...
def main()->int:
    parser=argparse.ArgumentParser(description="Orquestación reproducible G10");sub=parser.add_subparsers(dest="command",required=True)
    ...
    if args.command=="validate-plan":
        plan=read_json(args.plan);validate_plan(plan);print(f"G10 PLAN PASS: {plan['lot_id']} tasks={len(plan['tasks'])}")
