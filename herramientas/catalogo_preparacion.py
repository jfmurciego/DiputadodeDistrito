        ...
        for edition,state in row["editions"].items():
            ...
            if state.get("electoral_source_prepared") and electoral_evidence is not None:
                try:
                    ...
                    if receipt.get("schema")!="ddd.catalog-evidence/1.0" or receipt.get("kind")!="electoral_source":
                        errors.append(f"{tid}/{edition}: evidencia electoral con esquema/tipo inválido")
                    ...
                    errors.append(f"{tid}/{edition}: evidencia electoral inválida: {exc}")
