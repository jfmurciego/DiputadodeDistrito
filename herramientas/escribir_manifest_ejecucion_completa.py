...
def phase(
    ...
    return {
        ...
        "validation_decision": decision or None,
    }
...
def main() -> None:
    ...
    for prefix in ("territorial-source", "territorial-product", "electoral-source", "electoral-product"):
        ap.add_argument(f"--{prefix}-run-id")
        ap.add_argument(f"--{prefix}-artifact")
        ap.add_argument(f"--{prefix}-digest")
        ap.add_argument(f"--{prefix}-decision")

    ap.add_argument("--operational-state-result", required=True)
    ...
    phases = [
        phase(
            "01 · Preparación de Datos Territoriales",
            ns.prepare_territorial_result,
            b(ns.prepare_territorial_executed),
            ns.territorial_source_run_id,
            ns.territorial_source_artifact,
            ns.territorial_source_digest,
            ns.territorial_source_decision,
        ),
        phase(
            "02 · Generación de Distritos Autonómicos",
            ns.generate_result,
            b(ns.generate_executed),
            ns.territorial_product_run_id,
            ns.territorial_product_artifact,
            ns.territorial_product_digest,
            ns.territorial_product_decision,
        ),
        phase(
    ...
    payload = {
        "schema": "ddd.full-run-manifest/1.1",
