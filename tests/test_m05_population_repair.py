...
class PopulationRepairTests(unittest.TestCase):
    def run(self, assignments, units, adj, **kw):
    ...
    def test_simple_transfer_repairs(self):
        ...
        x=self.run(ass,units,A(("a1","a2"),("a2","b1"),("b1","b2")))
    ...
    def test_exchange_requires_two_moves(self):
        ...
        x=self.run(ass,units,A(("a1","a2"),("a2","b1"),("a1","b1"),("b1","b2")))
    ...
    def test_disconnect_donor_rejected(self):
        ...
        x=self.run(ass,units,A(("x","cut"),("cut","y"),("cut","z")),limits=r.SearchLimits(max_depth=1,max_transfer_set=1,max_candidates=50,max_seconds=1,seed=1))
    ...
    def test_municipal_integrity_rejected(self):
        ...
        x=self.run(ass,units,A(("m1","m2"),("m1","b")),limits=r.SearchLimits(max_depth=1,max_transfer_set=1,max_candidates=20,max_seconds=1,seed=1))
    ...
    def test_no_solution_within_budget_is_not_impossibility(self):
        ...
        x=self.run(ass,units,A(("a","b")),limits=r.SearchLimits(max_depth=1,max_transfer_set=1,max_candidates=1,max_seconds=1,seed=1))
    ...
    def test_deterministic_same_seed(self):
        ...
        self.assertEqual(self.run(ass,units,adj),self.run(ass,units,adj))

    def test_regression_monotonic_and_baseline_preserved(self):
        units={"a":U(100),"b":U(100)}; ass={"a":"A","b":"B"}; x=self.run(ass,units,A(("a","b")))
