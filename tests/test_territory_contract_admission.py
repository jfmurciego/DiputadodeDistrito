...
class PopulationRepairDeclarativeContract(unittest.TestCase):
    def setUp(self):
        self.path = ROOT / "territorios/castilla_y_leon/config/castilla_y_leon_2025.yaml"
        self.data = yaml.safe_load(self.path.read_text(encoding="utf-8"))

    def errors_after(self, mutate):
        data = copy.deepcopy(self.data)
        mutate(data["modulos"]["modulo_05_optimizar_distritos"])
        return population_repair_contract_errors(data)

    def test_perfil_generico_exacto_y_declarativo(self):
        m05 = self.data["modulos"]["modulo_05_optimizar_distritos"]
        self.assertEqual(m05["population_repair"], {
            "enabled": True,
            "max_depth": 4,
            "max_transfer_set": 2,
        ...
        self.assertNotIn("castilla_y_leon", source.lower())

    def test_limites_invalidos_y_parametros_desconocidos_se_rechazan(self):
        mutations = [
            lambda m: m["population_repair"].__setitem__("max_depth", 0),
            lambda m: m["population_repair"].__setitem__("max_transfer_set", 0),
            lambda m: m["population_repair"].__setitem__("max_candidates", 0),
            lambda m: m["population_repair"].__setitem__("max_seconds", 0),
            lambda m: m["population_repair"].__setitem__("territorial_override", 1),
        ]
    ...
    def test_desactivado_no_ejecuta_reparacion_ni_modifica_salida(self):
        wrapper_path = ROOT / "modulos/05_optimizar_distritos.py"
        spec = importlib.util.spec_from_file_location("ddd_m05_wrapper_contract_test", wrapper_path)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        with tempfile.TemporaryDirectory() as td:
            out = Path(td) / "baseline.bin"
            out.write_bytes(b"baseline-exacto")
            before = out.read_bytes()
            original_repair = module.repair
            original_merge = module._merge_report_metadata
            try:
                module.repair = lambda **kwargs: (_ for _ in ()).throw(AssertionError("repair no debe ejecutarse"))
                module._merge_report_metadata = lambda *args, **kwargs: None
                meta = module._apply_population_repair({}, {"population_repair": {"enabled": False}}, out, None)
            finally:
                module.repair = original_repair
                module._merge_report_metadata = original_merge
            self.assertEqual(out.read_bytes(), before)
            self.assertFalse(meta["enabled"])

    def test_aragon_no_activa_ni_declara_esta_capacidad(self):
        path = ROOT / "territorios/aragon/config/aragon_2025.yaml"
        data = yaml.safe_load(path.read_text(encoding="utf-8"))
        m05 = data["modulos"]["modulo_05_optimizar_distritos"]
        self.assertNotIn("population_repair", m05)
        self.assertNotIn("unit_id_field", m05)
