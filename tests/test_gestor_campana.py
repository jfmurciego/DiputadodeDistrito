...
def _write_structural_fixture(root: Path, *, b_sha: str = "b" * 64, authorization=...,
                              broken_k: bool = False) -> Path:
    ...
    contract.write_text(yaml.safe_dump({
        ...
        "modulos": {
            ...
            "modulo_04_generar_semillas": {
                "in_graph_json": "graph.json", "in_geojson": "source.geojson",
                "out_geojson": "seeds.geojson", "municipality_field": "CUMUN", "k_districts": 45,
            },
            "modulo_05_optimizar_distritos": {
                "in_graph_json": "graph.json", "in_geojson": "seeds.geojson",
                "out_geojson": "optimized.geojson", "municipality_field": "CUMUN",
            },
            "modulo_06_consolidar_distritos": {
                "in_geojson": "optimized.geojson", "municipality_field": "CUMUN",
                "expected_districts": 44 if broken_k else 45,
            },
        },
        "validation": {
    ...
    catalog.write_text(yaml.safe_dump({
        ...
        "territories": [{
            "territory_id": "fixture", "name": "Fixture",
            "editions": {"2025": {
                "territory_declared": True, "preparation_status": "READY",
                "contract_path": "contract.yaml", "territorial_source_declaration": "sources.yaml",
                "electoral_source_declaration": None, "territorial_contract_complete": True,
                "territorial_sources_prepared": True, "territorial_product_available": False,
                "electoral_source_prepared": False, "electoral_product_available": False,
                "territorial_certification": "NOT_CERTIFIED", "production_authorization": "AUTHORIZED",
...
def _test_effective_manifest_source_precedes_catalog_provenance(self):
    ...
    with tempfile.TemporaryDirectory() as raw:
        root = Path(raw)
        catalog = _write_structural_fixture(root, b_sha="b" * 64)
        invalid_a = {**source_a, "artifact_sha256": "invalid-A"}
        with self.assertRaisesRegex(ValueError, "CAP_SOURCE"):
            build_plan(
                territory="Fixture", edition="2025", execution_mode="reuse",
                catalog=catalog, root_dir=root, force_selected_algorithm=True,
                explicit_territorial_source=invalid_a,
            )
...
def _test_authorization_is_neutral_but_explicit_block_is_not(self):
    with tempfile.TemporaryDirectory() as raw:
        root = Path(raw)
        catalog = _write_structural_fixture(root, authorization="AUTHORIZED")
...
_core.CampaignManagerTests.test_effective_manifest_source_precedes_catalog_provenance = (
    _test_effective_manifest_source_precedes_catalog_provenance
)
_core.CampaignManagerTests.test_authorization_is_neutral_but_explicit_block_is_not = (
    _test_authorization_is_neutral_but_explicit_block_is_not
)
_core.CampaignManagerTests.test_explicit_source_never_repairs_missing_structural_capability = (
    _test_explicit_source_never_repairs_missing_structural_capability
)
